...
public sealed class TurkuazPromosyonScraper : ISiteScraper
{
    ...
    private async Task<List<ResultRow>> ExtractListingAsync(IPage page, Uri pageUri, string seedUrl)
    {
        ...
        var jsListing = @"() => {
            const results = [];
            document.querySelectorAll('.left, .product-block').forEach(item => {
                const nameEl = item.querySelector('h3.name a, .name a');
                if (!nameEl) return;
                ...
                // Fiyat
                const priceEl = item.querySelector('.price-gruop, .price .special-price');
