...
public sealed class TekinozalitScraper : ISiteScraper
{
    ...
    public async Task<List<ResultRow>> ExtractAsync(IPage page, Uri pageUri, string seedUrl)
    {
        ...
        try { await page.WaitForSelectorAsync("a[href*='/urun/']", new() { Timeout = 6000 }); }
        ...
        var items = await page.EvaluateAsync<string[][]>($$"""
            () => {
                ...
                document.querySelectorAll('a[href*="/urun/"]').forEach(a => {
                    ...
                    const minQty = card ? extractMinOrder(card.innerText || '') : '';
                    if (name.length > 2) results.push([href, name.slice(0, 150), price, minQty]);
                });
        ...
        foreach (var item in items ?? [])
        {
            ...
            rows.Add(ScraperHelpers.CreateRow(store, seedUrl, href, category, name, p, ccy, quote, kdv, minQty));
        }
        return rows;
    }
}